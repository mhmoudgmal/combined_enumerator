...
class CombinedOrderedEnumerator < Enumerator
  def initialize(*enumerators)
    super() do |yielder|
      ...
      loop do
        ...
        # 1 - get peeks, no forward moves for enumerators yet
        peeks = enumerators.map { |enumerator|
          begin
            enumerator.peek
        ...
        # 2 - out-[] if []-in
        break if peeks.empty?

        # 3 -   a- get min of peeks.
        #       b- get the corresponding enumerator to that min val,
        #       c- check that enumerator for order and then yield it.
        # ------------------------------------------------------------
        min     = peeks.reject(&:nil?).min
        min_idx = peeks.index(min)

        check_unordered(enumerators[min_idx])
  ...
  def check_unordered e
    raise UnorderedEnumerator.new(e) if e.first > e.next
  end
